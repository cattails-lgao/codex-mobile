...
- **round-52 发布进行中（v0.1.103）**：`codex-mobile-re@0.1.103`——live 多 agent 去重（源 `upsertLiveAgentMessage` + 最终汇合 `dedupeAssistantAgentMessageText`）+ 补充修复（`buildTurnRenderGroups` `liveOverlayActive` 双守卫，防止 live 流式中已完成中间消息被误提升为 final 拉出过程区）；真机 DOM 直证验证通过（线程 01a024d1 流式中中间消息保持 `conversation-item-process`、结束后真正最终正常落成 `conversation-item-final`）；全量单测 + `vue-tsc --noEmit` 通过，`pnpm run build` 通过（web + CLI 均成功）。**代码与 git tag `v0.1.103` 已提交并推送**（tag 推送到 `origin`）；**GitHub release `v0.1.103` 与 npm 发包均由用户手动执行**（本机无 `gh` CLI、无 `GITHUB_TOKEN`，无法自动创建 release；release 页面 https://github.com/cattails-lgao/codex-mobile/releases/new?tag=v0.1.103&title=v0.1.103 已可填正文并发布，tag/标题预填）。发布完成后用 `npm view codex-mobile-re@0.1.103` 确认 latest 后回填。
...
*codexapp · 交接文档 · 2026-08-21（round-52 多 agent 进行中重复 agentMessage 块 live 文本级去重修复；round-51 最终 assistant 块 + zen-proxy namespace 工具修复）· 内容已脱敏*
