...
| 项 | 值 |
|---|---|
| Git 分支 | main（round-75：侧边栏重复线程行去重——paginated rollout 分段同一 id 多行归并为最新段，v0.1.120 已发布闭环；与 `origin/main` 同步） |
| Dav 端口 | 4173 |
| Dev 状态 | dev server 已验证；round-75 以定向 Vitest 与 `vue-tsc`/`vite build`/`tsup` 验证；版本 0.1.120 已 tag/Release 且 npm publish 完成（`latest` = 0.1.120），发布链路闭环；不操作 5173 |
...
| 最近提交 | round-75：侧边栏重复线程行去重——paginated 线程回退后 app-server 从 `sessions/` 目录扫描构建 `thread/list`，把同一 session 的多段 rollout 当成多条线程返回（`state_*.sqlite` 仍是「一 id 一行」），侧边栏同一线程渲染多行并触发 duplicate-key。①bridge 层新增 `dedupeThreadListByIdKeepNewest`，作为 `thread/list` 终态按 id 归并、保留 `updatedAt` 最新段、保持首次出现顺序、无重复返回原引用（`8852718`）；②前端 `dedupeThreadGroupsById`（`src/utils/threadGroups.ts`）在四处渲染源兜底；③附 `rpcPipeline` 3 例与 `threadGroups` 4 例测试。版本 bump 至 0.1.120（`8852718` 修复 + `bfcf11b` 文档/版本），已 tag/Release 且 npm publish 完成（`latest` = 0.1.120） |
...
*codexapp · 交接文档 · 2026-09-11（round-75：侧边栏重复线程行去重——paginated rollout 分段同一 id 多行在 bridge 层按 id 归并保留 `updatedAt` 最新段，前端渲染源 `dedupeThreadGroupsById` 兜底，消除 duplicate-key 与重复行；定向 Vitest/`vue-tsc`/`vite build`/`tsup` 通过；版本 bump 至 0.1.120，tag/GitHub Release 与 npm publish 均已完成（`latest` = 0.1.120））· 内容已脱敏*
