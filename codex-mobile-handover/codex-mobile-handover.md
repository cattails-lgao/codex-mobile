...
| 最近提交 | round-45 release 随 v0.1.97（`7fa6ec4` 修复 thread/read 时序恢复把 reasoning 挤到轮末；`ad55eef` round-44 文档：WebUI 查看 TUI 会话的 writer-lock 已知限制；`b2074f3` 版本 0.1.97），GitHub release `v0.1.97` 已创建，`codex-mobile-re@0.1.97` 已发布到 npm（latest）；round-43/0.1.96（`ff6df2b`/`5aaa458`/`64fa15d`）已推送并发布 |
