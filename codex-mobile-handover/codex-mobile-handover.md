...
| 项 | 值 |
|---|---|
| Git 分支 | main（round-53 已完成 live agent turn 所有权保留与后台恢复卡顿优化；`codex-mobile-re@0.1.104` 已准备发布） |
| Dev 端口 | 4173 |
| Dev 状态 | 运行中 · HTTP 200 |
| App-server | 正常响应 RPC |
...
| 最近提交 | **v0.1.104 发布准备**：收录 round-53 的 `2f9643b`（live agent 按所属 turn 归位，防止上一轮消息混入新轮）与 `e74ab73`（恢复前台不等待附属元数据，`thread/list` 使用 tracker 缓存）。104 个相关单测、类型检查和完整构建通过；GitHub Release 与 npm publish 待本轮分别完成。详见 [round-53](rounds/round-53-live-turn-ownership-and-resume.md) |
...
- **v0.1.104 发布准备（2026-08-25 round-53）**：收录两项已完成修复。`2f9643b` 为 live delta/completed agent 消息保留通知 `turnId`、解析对应 `turnIndex`，在消息流中插回所属持久化轮次；`ThreadConversation` 传递实际 `liveTurnId`，故只有活跃轮抑制 final 提升，上一轮迟到消息不会混入新轮。`e74ab73` 切除恢复关键路径的两类阻塞：`thread/list` 直接使用 external-session tracker 最近完成扫描的缓存（后台轮询更新，不在 RPC 内等待递归扫描），移动端/后台标签恢复不再等待 skills、限额、协作模式等附属元数据。104 个相关单测、`vue-tsc --noEmit` 与 `pnpm run build` 已通过；性能审计为代码路径级：移除一项 RPC 同步扫描及一次恢复等待，未新增请求、轮询、缓存体积或无界扇出；本轮未重采 profile。版本升至 **`0.1.104`**，GitHub Release 由本轮创建，npm publish 由用户执行。详见 [round-53](rounds/round-53-live-turn-ownership-and-resume.md) 与 [合并手测项](../tests/thread-loading-state/live-agent-turn-ownership-and-resume-responsiveness.md)。
