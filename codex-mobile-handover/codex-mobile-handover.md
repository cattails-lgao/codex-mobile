...
| 项 | 值 |
|---|---|
| Git 分支 | main（`v0.1.106` 待 GitHub Release；round-57/58 修复最终总结在过程收尾和完成态竞态下的归属） |
| Dev 端口 | 4173 |
...
| 最近提交 | `0e0d0e6`：完成态下保留最终总结；本轮将版本升至 `0.1.106` 并创建 GitHub Release |
...
- **v0.1.106 发布（2026-08-26 round-59）**：收录 round-57（过程收尾记录不再遮蔽最终总结）和 round-58（`turn/completed` 先到、最终消息暂留 `.live` 时总结不再回落到过程区）。版本升至 **`0.1.106`**；GitHub Release 由本轮创建，npm publish 由用户执行。针对性回归 127/127、`vue-tsc --noEmit` 和生产构建均通过。详见 [round-59](rounds/round-59-v0.1.106-release.md)。
