...
| 最近提交 | `fdbedb8`：round-61 v0.1.107 线程状态修复已提交并推送；tag 与 GitHub Release `v0.1.107` 已创建。npm publish 因 npm 官方源 401 未完成，需重新登录后重试；生产构建与合并定向 Vitest 58/58 已通过，浏览器手测未做 |
...
- **round-61 v0.1.107 线程状态修复（2026-08-27，npm 待发布）**：版本已升至 **`0.1.107`**。修复 `thread/list` 在异步后处理期间 tracker 扫描完成却仍使用旧子代理排除快照的问题；修复新 live overlay 出现但 `liveTurnId` 尚未知时，上一轮最终总结被误判为活跃轮过程项的问题。两项均已补最小 Vitest 回归用例，并更新线程加载/状态手测说明；生产构建 `pnpm run build` 已通过，合并定向 Vitest（`rpcPipeline.test.ts`、`externalSessionTracker.test.ts`、`transcriptGrouping.test.ts`、`transcriptGrouping.repro.test.ts`）**58/58** 通过；浏览器手测未做。修复提交 `fdbedb8` 已推送，tag 与 GitHub Release `v0.1.107` 已创建；npm publish 因 npm 官方源 401 未完成，重新登录后重试。详见 [round-61](rounds/round-61-v0.1.107-thread-state-fixes.md)。
...
*codexapp · 交接文档 · 2026-08-27（round-61：版本 `0.1.107` 与两项线程状态修复已由 `fdbedb8` 提交并推送，tag 与 GitHub Release 已创建；npm 官方源 401，重新登录后待发布。生产构建通过、合并定向 Vitest 58/58 通过，浏览器手测未做）· 内容已脱敏*
