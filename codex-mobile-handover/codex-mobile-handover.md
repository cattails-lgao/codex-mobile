...
| 最近提交 | round-45 release 随 v0.1.97（`7fa6ec4` 修复 thread/read 时序恢复把 reasoning 挤到轮末；`ad55eef` round-44 文档：WebUI 查看 TUI 会话的 writer-lock 已知限制；`b2074f3` 版本 0.1.97），GitHub release `v0.1.97` 已创建，`codex-mobile-re@0.1.97` npm 发布待维护者登录执行（token 已失效）；round-43/0.1.96（`ff6df2b`/`5aaa458`/`64fa15d`）已推送并发布 |
...
- **已推送**：`main` 与 `origin/main` 已同步至 `64fa15d`（round-41/42 修复 `e1dccb9`/`a33395e`/`548983e`/`6378b34` 随 v0.1.95 发布，tag `v0.1.95`；round-43 修复 `ff6df2b`/`5aaa458` + 版本 `64fa15d` 随 v0.1.96 发布，tag `v0.1.96`；round-44 文档 `ad55eef` + round-45 修复 `7fa6ec4` + 版本 `b2074f3` 随 v0.1.97 发布，tag `v0.1.97`，`origin/main` 已同步）。推送方式：优先直连 GitHub（偶发成功）；直连失败时临时经本机代理 `git -c http.proxy=socks5h://127.0.0.1:10808 -c https.proxy=socks5h://127.0.0.1:10808 push`，未改全局 git 配置。注意：代理端口（10808/10811/10812）以 xray/v2rayN 进程是否存活为准，退出后端口即失效，直连即可（2026-08-06 晚实测代理全关、直连重试 3 次后成功）。2026-08-11 round-34/35 推送直连成功（git 2.35.1，位于 `E:\Git`，不在默认 PATH，需前置进 PATH 或全路径调用）。gh release 需 `--repo cattails-lgao/codex-mobile` 指定本 fork（默认会命中 upstream remote）；npm 401 需重新登录（token 已失效），prepublishOnly 会执行 `pnpm run build`，发包前把 `<pnpm 全局 bin 目录>` 加入 PATH
