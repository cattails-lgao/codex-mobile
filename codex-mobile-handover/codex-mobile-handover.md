...
| Dev 端口 | 4173 |
| Dev 状态 | dev server 已验证（本轮以真实最重线程 `01a0465a-f570-78a0-ae6b-badc1c40b4e8` 做改动前后同口径对比）；round-76 以定向 Vitest + 全量 Vitest + `vue-tsc`/`vite build` 验证；版本 0.1.121；不操作 5173 |
...
| 最近提交 | round-76：线程/消息列表加载性能优化。基线实测最重线程 `thread/read` 13.55MB（19 轮未裁剪）、前端打开 3041.8ms/4.29MB、首屏 `/codex-api` 共 4,448.4KB，其中 92% 是 UI 从不整段渲染的大文本（`commandExecution` 74%、`mcpToolCall.result` 18%）。四项桥层改动：①删前端从不读取的 `mcpToolCall.result`；②命令输出超 16KB 中段截断（留头 60%+尾 40%）+ sha1 落盘 + 新增 `GET /codex-api/command-output?ref=` 按需回读，前端加截断提示行与「查看完整输出」入口；③`thread/read` 结果缓存（TTL 20s/≤6 条/变更即失效，不缓存 `thread/resume`），重复读 1745ms → 72ms；④app-server 启动预热（实测只能部分重叠冷启动，价值在后续加载，已如实写进注释）。实测最重线程响应体 **4,391.5KB → 1,599.1KB（−63.6%）**、首屏总字节 **−54.8%**。新增 3 个测试文件（`payloadSlimming`/`threadReadCache`/`commandOutputRoute`）+ `v2.test.ts` 3 例；全量 569 通过/2 失败（既有 Windows 环境性失败）。版本 bump 至 0.1.121 |
...
*codexapp · 交接文档 · 2026-09-11（round-76：线程/消息列表加载性能优化——桥层删未消费的 `mcpToolCall.result`、命令输出 16KB 内联截断 + sha1 落盘 + 按需回读路由、`thread/read` 结果缓存、app-server 启动预热；实测最重线程响应体 4,391.5KB → 1,599.1KB（−63.6%）、首屏 `/codex-api` 总字节 −54.8%、重复读 1745ms → 72ms；全量单测 569 通过 / 2 既有 Windows 环境性失败，`vue-tsc` 与 `vite build` 通过；版本 bump 至 0.1.121，代码 `0d18ca2` / 文档 `3e6405c` / 哈希记录 `449cbc1` 已推送 `origin/main`，tag/Release 与 npm publish 待执行）· 内容已脱敏*
