...
| 最近提交 | round-43 修复 `ff6df2b`/`5aaa458`（侧边栏泄漏子 agent 线程——`externalSessionTracker` subagent 用自身 id 键控；模型强度下拉收敛到 Low/Medium/High，待推送），上轮 6378b34（round-41/42 修复：自定义端点 URL 归一化与模型列表、回退回填、codex 模型对齐 codex-cli；随 v0.1.95 发布） |
...
- **待推送**：round-43 修复（`ff6df2b` 侧边栏泄漏子 agent 线程——subagent 用自身 `id` 键控；`5aaa458` 模型强度下拉收敛到 Low/Medium/High）已 commit，尚未 push；按上面「推送方式」推送到 `origin/main`（未随 v0.1.95，需后续发版带上）
