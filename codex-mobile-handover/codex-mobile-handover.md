...
*codexapp · 交接文档 · 2026-09-11（round-75：侧边栏重复线程行去重——paginated rollout 分段同一 id 多行在 bridge 层按 id 归并保留 `updatedAt` 最新段，前端渲染源 `dedupeThreadGroupsById` 兜底，消除 duplicate-key 与重复行；定向 Vitest/`vue-tsc`/`vite build`/`tsup` 通过；版本 bump 至 0.1.120，tag/GitHub Release 与 npm publish 待执行）· 内容已脱敏*
