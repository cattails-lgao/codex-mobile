...
| 项 | 值 |
|---|---|
| Git 分支 | main（round-76：线程/消息列表加载性能优化——桥层载荷瘦身 + `thread/read` 结果缓存 + app-server 启动预热，v0.1.121；与 `origin/main` 同步） |
| Dev 端口 | 4173 |
| Dev 状态 | dev server 已验证（本轮以真实最重线程 `01a0465a-f570-78a0-ae6b-badc1c40b4e8` 做改动前后同口径对比）；round-76 以定向 Vitest + 全量 Vitest + `vue-tsc`/`vite build` 验证；版本 0.1.121 已 tag/Release（`v0.1.121` 为 Latest）且 `codex-mobile-re@0.1.121` 已 publish 至 npm（`latest` = 0.1.121），发布链路闭环；不操作 5173 |
...
*codexapp · 交接文档 · 2026-09-11（round-76：线程/消息列表加载性能优化——桥层删未消费的 `mcpToolCall.result`、命令输出 16KB 内联截断 + sha1 落盘 + 按需回读路由、`thread/read` 结果缓存、app-server 启动预热；实测最重线程响应体 4,391.5KB → 1,599.1KB（−63.6%）、首屏 `/codex-api` 总字节 −54.8%、重复读 1745ms → 72ms；全量单测 569 通过 / 2 既有 Windows 环境性失败，`vue-tsc` 与 `vite build` 通过；版本 bump 至 0.1.121，代码 `0d18ca2` / 文档 `3e6405c` / 哈希记录 `449cbc1` 已推送 `origin/main`；git tag `v0.1.121` 与 GitHub Release 已创建（Latest），`codex-mobile-re@0.1.121` 已 publish 至 npm 并成为 `latest`，发布链路全部闭环）· 内容已脱敏*
