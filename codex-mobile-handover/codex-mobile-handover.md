...
| 项 | 值 |
|---|---|
| Git 分支 | main（round-78：线程打开时延根治——轮耗时镜像只发增量 + provider-models 并发去重 + provider 目录服务端缓存与启动预热，v0.1.122；与 `origin/main` 同步） |
| Dev 端口 | 4173 |
| Dev 状态 | dev server 已验证（round-78 用 codegraph 图谱复查后，以真实 home 环境做同口径 A/B：短路 provider-models 首次点击 1102→126ms，据此定位并修复；探针 `output/playwright/perceived-latency.cjs` 已扩展时间线与 `PROFILE_FAKE_GIT`/`PROFILE_FAKE_MODELS` 开关）；v0.1.122 以定向 Vitest（round-77 新增 3 例 + round-78 新增 6 例）+ 全量 Vitest 578 通过/2 既有 Windows 环境性失败 + `pnpm run build`（`vue-tsc --noEmit` + `vite build` + `tsup` CLI）通过验证；版本 0.1.122 已 tag/Release，**npm publish 由用户执行**；不操作 5173 |
...
*codexapp · 交接文档 · 2026-09-11（round-78：接入 `@colbymchenry/codegraph` MCP（稳定启动器 `~/.workbuddy/bin/codegraph-mcp.cjs` + `~/.workbuddy/mcp.json`，握手已验证，**需在连接器管理页「信任」后加载**）+ 用图谱复查线程打开时延——首次点击固定 ~1.1s 与线程内容无关，时间线显示 `thread/resume` 仅 32ms 却等到 +1221ms 才发出（紧随 `provider-models` 结束）；A/B 归因单独短路 `/codex-api/provider-models*` → 1102→126ms，短路 `free-mode/status` 或 `/codex-api/git/**` 均无变化；根因是 `selectThread` 在关键路径上 await 外部 provider 目录，而服务端 zen/custom 目录请求无缓存、openrouter 目录请求只在成功时写缓存且其 `fetch` 无超时。改动 3 文件：目录缓存（TTL 10min／空结果 1min／in-flight 去重／过期先发后用）+ 失败记忆化 + 1.5s 超时 + `warmProviderModelCatalog()` 启动预热，实测 `provider-models` 1276ms → 3ms、首次点击 1102ms → ~112ms（−90%）。**v0.1.122 一并收录 round-77**（轮耗时镜像只发增量消除 PUT 写放大 + provider-models 并发去重：`thread/resume` 224ms → 81ms、点击线程 −47%~−65%、`thread-turn-durations` 请求 28 → 1）。验证：定向 Vitest 9 例、全量 **578 通过 / 2 既有 Windows 环境性失败**（`codexAppServerBridge.archive.test.ts`：symlink 规范化顺序、文件 mode `0o666` vs `0o600`）、`pnpm run build`（`vue-tsc --noEmit` + `vite build` + `tsup`）通过；版本 bump 至 **0.1.122**，git tag `v0.1.122` 与 GitHub Release 由维护者创建，**npm publish 由用户执行**）· 内容已脱敏*
