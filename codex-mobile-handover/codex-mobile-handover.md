...
*codexapp · 交接文档 · 2026-09-11（round-76：线程/消息列表加载性能优化——桥层删未消费的 `mcpToolCall.result`、命令输出 16KB 内联截断 + sha1 落盘 + 按需回读路由、`thread/read` 结果缓存、app-server 启动预热；实测最重线程响应体 4,391.5KB → 1,599.1KB（−63.6%）、首屏 `/codex-api` 总字节 −54.8%、重复读 1745ms → 72ms；全量单测 569 通过 / 2 既有 Windows 环境性失败，`vue-tsc` 与 `vite build` 通过；版本 bump 至 0.1.121，提交已推送、tag/Release 与 npm publish 待执行）· 内容已脱敏*
