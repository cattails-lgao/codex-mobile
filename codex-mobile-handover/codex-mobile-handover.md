...
| 最近提交 | round-46 release 随 v0.1.98（`6c75b90` round-45 文档版本 0.1.97；`24a23dc` 4 项 H5/UI 打磨：diff 弹窗 Teleport 到 body + z-[1200] 修复被顶栏遮挡/无法关闭、模型按钮改 w-fit+max-w、命令块 header 改「序号+命令+状态」并把具体命令移入展开输出区；`47f3326` 版本 0.1.98 + round-46 交接文档）。tag `v0.1.98` 已本地创建、main/tag 尚未推送、GitHub release `v0.1.98` 未创建、npm 未发包——2026-08-18 晚 GitHub 网络不可达（直连 443 与代理 10808/10811/10812 均超时/拒绝），待网络恢复后补推送+建 release；npm `codex-mobile-re@0.1.98` 待用户发布 |
...
- **已推送**：`main` 与 `origin/main` 已同步至 `64fa15d`（round-41/42 修复 `e1dccb9`/`a33395e`/`548983e`/`6378b34` 随 v0.1.95 发布，tag `v0.1.95`；round-43 修复 `ff6df2b`/`5aaa458` + 版本 `64fa15d` 随 v0.1.96 发布，tag `v0.1.96`；round-44 文档 `ad55eef` + round-45 修复 `7fa6ec4` + 版本 `b2074f3` 随 v0.1.97 发布，tag `v0.1.97`，`origin/main` 已同步）。**round-46/0.1.98 未推送**（`24a23dc` fix + `47f3326` 版本/文档，tag `v0.1.98` 已本地创建）：2026-08-18 晚 GitHub 网络不可达，直连 `github.com:443` 超时、代理 `socks5h://127.0.0.1:10808/10811/10812` 均 Connection refused（xray/v2rayN 未运行），待网络恢复后执行 `git push origin main` + `git push origin v0.1.98` 再建 GitHub release。推送方式：优先直连 GitHub（偶发成功）；直连失败时临时经本机代理 `git -c http.proxy=socks5h://127.0.0.1:10808 -c https.proxy=socks5h://127.0.0.1:10808 push`，未改全局 git 配置。注意：代理端口（10808/10811/10812）以 xray/v2rayN 进程是否存活为准，退出后端口即失效，直连即可（2026-08-06 晚实测代理全关、直连重试 3 次后成功）。2026-08-11 round-34/35 推送直连成功（git 2.35.1，位于 `E:\Git`，不在默认 PATH，需前置进 PATH 或全路径调用）。gh release 需 `--repo cattails-lgao/codex-mobile` 指定本 fork（默认会命中 upstream remote）；npm 401 需重新登录（token 已失效），prepublishOnly 会执行 `pnpm run build`，发包前把 `<pnpm 全局 bin 目录>` 加入 PATH
