...
| 最近提交 | `c588bdb`：v0.1.106 发布准备；GitHub Release 已创建，npm publish 由用户执行 |
...
- **v0.1.106 发布（2026-08-26 round-59）**：收录 round-57（过程收尾记录不再遮蔽最终总结）和 round-58（`turn/completed` 先到、最终消息暂留 `.live` 时总结不再回落到过程区）。版本升至 **`0.1.106`**；GitHub Release 已创建，npm publish 由用户执行。针对性回归 127/127、`vue-tsc --noEmit` 和生产构建均通过。详见 [round-59](rounds/round-59-v0.1.106-release.md)。
