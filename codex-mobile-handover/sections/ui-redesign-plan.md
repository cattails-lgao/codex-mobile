...
> **实现（round-96）**：阶梯落地为 `@theme` token `--text-nano/micro/ui/body/display`（nano 10px 为度量稿实际在用的徽记补充档）；token 只定字号、不绑行高（与被替换的 `text-[Npx]` 逐字同行为，行高由使用点搭配）；12px 复用 `text-xs`、18/24px 复用 `text-lg/text-2xl`。全仓 `text-[Npx]` 已清零，契约断言「清零」。
...
**P1 进度 ④（2026-09-24，见 [round-94](../rounds/round-94-code-floor.md)）——代码块楼层（语言标签 + 复制）**
- ✅ **楼层形态**（度量稿 `pre`）：`--s2` 表面 + `--line-1` 发丝边 + 14px 圆角，**跟随主题**（原先两套主题下都是 `bg-slate-950` 深底的「永远的黑」）；代码区 `--ink-2` / 13px / 1.65 行高。
- ✅ **bar 顶栏**：语言标签（等宽 11px 大写 `--ink-3`）与复制控件合入 `--s1` 底 + 发丝下边的顶栏，语言左 / 复制右。
- ✅ **复制**：`span[role=button][data-code-copy]` + `conversation-root` 根级 click/keydown **事件委托**——模板路径与 `ReasoningBlock` 的 v-html 路径同一份委托（sanitizer 白名单剥 `button`，`span`+`role`+`data-*` 可存活）；成功态「已复制」1.6s 复位（`--ok` 色）。
- ✅ 删全局暗色覆盖 2 条（round-92/93 同款理由）；契约 26 → **28 项**（楼层只用 token、复制控件两路都在）；等值采样点 20 → **23**、基线 114 → **120 元素 / 720 属性**——先武装后改码，改动被如实报出 **15 项且零泄漏**（全部落在 3 个代码块采样点 × 2 页）。
- ⏸ **剩余 P1**：会话（Markdown 标题层级）、线程行（等宽相对时间 + 运行 pip + 导轨）、侧栏其余入口降噪。

**P1 进度 ⑤（2026-09-24，见 [round-95](../rounds/round-95-headings-thread-rows.md)）——标题层级 + 线程行 + 区头降噪（P1 会话/侧栏清单清零）**
- ✅ **Markdown 标题层级**：h1-h4 = 标准档 20/18/16/14px（`text-[Npx]` 计数不增），h5/h6 并入 14px 靠字距；全部 `text-ink-1`，`#17181a` 硬编码清零。层级靠字号与字距，不靠色相。过程消息的 15px 弱化是有意次级样式，保留。
- ✅ **线程行（度量稿 `.row`/`.when`/`.pip`）**：hover/选中抬 `bg-s2`；**选中导轨** 2px 中性（`ink-2`）；**运行导轨** `data-live` 时转 `--live`（琥珀只表示「在动」）；**运行 pip** = spinner 退场，6px `bg-live` + 22% 光晕 + 呼吸（reduced-motion 归零）；**相对时间** = 等宽 12px `--ink-3` + `tabular-nums`。
- ✅ **侧栏区头降噪**：`section-toggle-row` / `chats-section-action` / `organize-menu-trigger` 的 zinc 裸类 → `s2`/`ink-3`/`line-2`；删冗余暗色覆盖 5 条。
- ✅ 契约 28 → **32 项**；等值采样点 23 → **27**、基线 120 → **145 元素 / 870 属性**——先武装后改码，改动被如实报出 **43 项且零泄漏**（全部落在 4 个目标采样点）。
- ✅ **P1 剩余清单至此清零**（会话区签名组件、代码块楼层、标题层级、输入区、线程行、侧栏入口全部落地）。

**P2 · 亮色与无障碍对齐**（round-96/97 完成，2026-09-24；用户免审授权「直接从头到尾改完」）
...
**待决事项已清零（2026-09-24 定案）**：**亮色 token 的取值＝方案原值（中性）**。依据：该套值早已是代码中 `:root` 块的实际取值（§4 注记），度量稿（用户已认可）即按此渲染——待决的从来不是「选哪套值」，而是「把消费者迁过来」（round-97 已完成，亮侧裸色板清零）。原两候选留档：
