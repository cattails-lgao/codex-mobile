...
- 2026-08-28：**useDesktopState() 消息历史加载（消息加载读请求+缓存所有权）拆分完成**。`useDesktopState.ts` 自 ~3,787 行按领域再拆一批（-201 行，新增模块 46 行引用）。
  - 新建 `src/composables/useDesktopMessageHistoryLoading.ts`：按「窄依赖注入」模式，将 `loadMessages`/`loadOlderMessages`/`ensureThreadMessagesLoaded` 的**读请求与缓存所有权**收敛于此模块：
    - **自有缓存 ref**：`loadedVersionByThreadId`/`loadedMessagesByThreadId`/`hasMoreOlderMessagesByThreadId`/`loadingOlderMessagesByThreadId`/`isLoadingMessages`。
    - **自有请求合并缓存**：`loadMessagePromiseByThreadId`/`lastMessageLoadAtByThreadId`/`lastMessageLoadFailureAtByThreadId`（含 `RECENT_THREAD_MESSAGE_LOAD_REUSE_MS` 复用窗口逻辑，随迁）。
    - **依赖注入（写侧编排保留在主状态闭包）**：`selectedThreadId`/`error`/`persistedMessagesByThreadId`/`liveAgentMessagesByThreadId`/`inProgressById`/`externalSessionByThreadId`/`resumedThreadById`/`turnIndexByTurnIdByThreadId`/`turnErrorByThreadId`/`activeTurnIdByThreadId` 十个 ref + 写侧 helper（`markThreadAsRead`/`markThreadMessagesPersisted`/`replaceTurnIndexLookupForThread`/`rebindLiveFileChangeTurnIndices`/`setPersistedMessagesForThread`/`setLiveAgentMessagesForThread`/`clearLiveAgentMessagesForThread`/`removeLiveCommandsPersistedIn`/`removeLiveFileChangesPersistedIn`/`setThreadInProgress`/`setThreadModelId`/`setThreadModelProviderId`/`resolveThreadModelForProvider`/`clearTransientTurnErrorForThread`/`clearCompletedTurnLiveState`/`setTurnErrorForThread`/`getFirstPersistedTurnId`/`currentThreadVersion`）。纯函数 `mergeMessages`/`hasOptimisticUserMessages`/`removeRedundantLiveAgentMessages`/`omitKey`/`pruneThreadStateMap` 直接 import，网关 `resumeThread`/`getThreadDetail`/`getOlderThreadMessages` 直接 import。
    - 模块返回自有 ref + 四函数 + `pruneMessageHistoryState`（收敛原 `pruneThreadStateMap` 对 cached ref 的两处剪枝）。
  - 主文件：删除缓存 ref/isLoadingMessages/三张请求合并 Map/三函数定义，改为 `createDesktopMessageHistoryLoading({...deps})` 后解构出同名 ref 与函数；`pruneThreadScopedState` 内两行剪枝收敛为 `pruneMessageHistoryState(activeThreadIds)`。下游调用点（`selectThread`/`syncFromNotifications`/`recoverBridgeState`/fork handler/各返回导出）零改动。未触碰 live turn / 最终总结 / realtime 时序逻辑。
  - 验证：`vue-tsc --noEmit` 通过、`useDesktopState.test.ts` 89 个单测通过、全量单测通过（唯一 2 个失败均在 `codexAppServerBridge.archive.test.ts` 的 filesystem 用例——workspace roots 路径规范化与 CODEX_HOME 目录创建，受沙箱临时目录写入限制，与本批无关且非改动文件）、`vite build` 前端构建通过。
- 2026-08-28：**useDesktopState() 线程标题缓存（读请求+缓存所有权）拆分完成**。`useDesktopState.ts` 自 ~3,632 行再拆一批。
  - 新建 `src/composables/useDesktopThreadTitleCache.ts`：自有缓存 ref `threadTitleById`，读/归一化函数 `applyCachedTitlesToGroups`/`loadThreadTitleCacheIfNeeded`/`resolveFallbackThreadTitle` 与异步生成 `requestThreadTitleGeneration`。标题生成落地后需刷新线程 flags，该写侧编排 `applyThreadFlags` 经窄依赖注入（`ThreadTitleCacheDeps`）避免循环；`getThreadTitleCache`/`generateThreadTitle`/`persistThreadTitle` 网关直接 import，`toOptimisticThreadTitle`/`OPTIMISTIC_THREAD_TITLE_MAX` 从 utils import。
  - 主文件：删除 `threadTitleById` ref 与四函数定义，改为 `createDesktopThreadTitleCache({ applyThreadFlags })` 后解构同名引用；rename 写点（`thread/name/updated` 通知 + `renameThreadById`）继续直接改写返回的 `threadTitleById` ref 并自行 `persistThreadTitle`；`loadThreadTitleCacheIfNeeded` 继续注入 `threadListLoading` deps。清理随迁后不再使用的 import（`getThreadTitleCache`/`generateThreadTitle`/`OPTIMISTIC_THREAD_TITLE_MAX`）。下游调用点零改动。
  - 验证：`vue-tsc --noEmit` 通过、`useDesktopState.test.ts` 89 个单测通过、全量单测 424/426（仅 2 个无关 server 文件系统用例受沙箱限制）、`vite build` 前端构建通过。
- 2026-08-28：**useDesktopState() 待办服务端请求（pending server requests 读请求+缓存所有权）拆分完成**。`useDesktopState.ts` 自 ~3,555 行降至 ~3,483 行。
