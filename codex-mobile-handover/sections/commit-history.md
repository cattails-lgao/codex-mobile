...
- **round-43**：侧边栏泄漏子 agent 线程——`externalSessionTracker.updateSessionMeta` 对 subagent rollout 改用自身 `id` 键控（`session_id` 是父线程 id），使 `getSubagentThreadIds` 过滤正确的子线程 id、`externalSession` 叠加挂到子线程行、`filterThreadListByIds` 不再误删父行（`ff6df2b`）；模型强度下拉档位收敛到 Low/Medium/High（`reasoningOptionCatalog` 8→3，provider-only 模型不再冒出 Ultra 等项，`5aaa458`）（详见 `rounds/round-43-feedback.md`）
