...
随 **v0.1.124** 发布。commit 链、tag 与 GitHub Release 记录见 [sections/commit-history.md](../sections/commit-history.md) 的 v0.1.124 段；npm publish **用户决定暂缓**（2026-09-15 复查 registry：`dist-tags.latest` 仍为 `0.1.123`，`0.1.124` 尚未发布）。**（2026-09-16 补记：`0.1.124` 始终未 publish 至 npm，其内容已并入 `v0.1.125` 一并发布。）**
