...
> **环境注意：** 本机 PATH 中无 `node.exe`（版本管理器管理），直接运行 `pnpm` 会报「node 无法识别」。验证命令需先加入 node 目录：`$env:PATH = '<node 安装目录>;' + $env:PATH`。另外本会话添加了 `upstream` remote（`friuns2/codex-mobile`），直连 fetch 超时未成功，未影响使用。
