...
Merged from https://github.com/DietrichGebert/ponytail (AGENTS.md, MIT license). This is the first-pass coding rule for every task in this repo. If it conflicts with a repo-specific section below, the repo-specific section wins (e.g. mandatory performance audits, `tests/<domain>/` manual test docs, Playwright verification, CJS smoke test).
...
Lazy code without its check is unfinished: non-trivial logic leaves ONE runnable check behind, the smallest thing that fails if the logic breaks (an assert-based demo/self-check or one small test file; no frameworks, no fixtures). Trivial one-liners need no test. In this repo, that check follows the Tests And Verification section below (keep `tests.md` as the root index, add only the relevant manual test entry).
