...
8. Throttle the RPC request, click `Reload`, then trigger a hook while that reload is in flight and confirm only one `hooks/list` request is active.
9. Block the reload RPC in DevTools, click `Reload`, then unblock it and confirm the previous hook rows remain visible after the loading state clears.
...
- Concurrent reloads reuse one request; a failed reload preserves the last successful list and restores the enabled reload button.
