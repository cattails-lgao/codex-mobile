...
#### Feature/Change Name
Composer 的规划模式/审批策略按钮显示当前选中值；审批策略保存后真正作用于运行中的 app-server（unless-trusted 下非信任命令弹审批确认）；模型强度默认 Medium，并在发送后的异步配置刷新中保留用户手动选择；thinking 中编辑消息确认后先停止当前会话再回滚。
...
7. 将 `Thinking` 切换为 `High`，发送一条消息；等待消息进入进行中状态与其后的异步刷新完成，确认下拉仍显示 `High`。切换到不支持 High 的模型时，强度应改为该模型支持的默认值。
8. 会话进入 thinking 后，点击一条历史用户消息的编辑按钮 → 确认弹窗 → 点确认 → 会话先停止（停止按钮/spinner 消失），再进入编辑草稿回填
...
- `Thinking` 下拉默认 `Medium`；同一模型上手动选择 `High` 后发送消息不会被全局配置的 `Medium` 覆盖
